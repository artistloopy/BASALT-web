  ...
  return Promise.all(
    breakpoints.map(async (w: number) => {
      try {
        ...
        // If getImage fails (often due to fetching remote images during build),
        // fallback to returning the original src string so build can continue.
        const fallbackSrc = typeof image === 'string' ? image : (image as ImageMetadata)?.src;
        return {
